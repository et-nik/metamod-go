...
typedef struct mutil_funcs_t
{
	...
	const char *    (*pfnGetGameInfo)           (plid_t plid, enum ginfo_t tag);
	...
	void            (*pfnGetHookTables)         (struct plid_t plid, struct enginefuncs_t **peng, struct DLL_FUNCTIONS **pdll, struct NEW_DLL_FUNCTIONS **pnewdll);
} mutil_funcs_t;
